...
2. Выберите ваше приложение → OAuth2 → Redirects
3. Добавьте: `https://ВАШ_ДОМЕН.railway.app/api/auth/discord/callback`
4. Scopes: `identify`, `guilds.members.read`
...
4. Или используйте @userinfobot — перешлите любое сообщение из группы
...
1. Модератор пишет `/auth` в Telegram-бот
2. Бот присылает ссылку «Войти через Discord»
3. Модератор авторизуется → Railway проверяет роль на сервере
4. Если роль **Модератор/Admin/Администратор** найдена — доступ открыт
5. Сессия действует **7 дней**, потом нужно повторить `/auth`
