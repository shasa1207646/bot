...
<div id="tab-home" class="tab active">
  <div class="hero">
    ...
    &nbsp;
    <button class="btn btn-accent" onclick="switchTab('ai', document.querySelector('.nav-btn:nth-child(5)'))">
      🤖 Спросить AI
    </button>
  </div>
...
<div id="tab-curator" class="tab">
  ...
  <div class="card">
    ...
    <div class="form-group">
      ...
      <textarea id="cur-experience" rows="3" placeholder="Расскажите об опыте администрирования, модерации других серверов..."></textarea>
    </div>

    <div class="form-group">
      <label>Как ты встретишь нового участника сервера? *</label>
      <textarea id="cur-welcome" rows="3" placeholder="Опишите, как вы будете встречать и помогать новым участникам..."></textarea>
    </div>

    <div class="form-group">
      <label>Твоя сильная сторона как куратора? *</label>
      <textarea id="cur-strength" rows="3" placeholder="Напишите свои сильные стороны..."></textarea>
    </div>

    <div class="form-group">
      <label>Почему хотите стать куратором? *</label>
      <textarea id="cur-motivation" rows="3" placeholder="Напишите свою мотивацию..."></textarea>
    </div>
...
// ─── Отправка заявки куратора (ИСПРАВЛЕНО) ────────────────────
async function submitCurator() {
  ...
  const welcome = document.getElementById('cur-welcome').value.trim();
  const strength = document.getElementById('cur-strength').value.trim();
  ...
  if (!username || !age || !name || !activity || !experience || !welcome || !strength || !motivation || !rules) {
    showAlert(alert, 'error', '⚠️ Заполните все обязательные поля.');
    return;
  }
  ...
  // Комбинируем все поля в experience для бэкенда
  const combinedExperience = `Опыт модерации:\n${experience}\n\nКак встречу участника:\n${welcome}\n\nСильная сторона:\n${strength}`;
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      // Очистка формы
      document.getElementById('cur-username').value = '';
      document.getElementById('cur-age').value = '';
      document.getElementById('cur-name').value = '';
      document.getElementById('cur-activity').value = '';
      document.getElementById('cur-games').value = '';
      document.getElementById('cur-experience').value = '';
      document.getElementById('cur-welcome').value = '';
      document.getElementById('cur-strength').value = '';
      document.getElementById('cur-motivation').value = '';
      document.getElementById('cur-rules').value = '';
      document.getElementById('cur-check').className = 'discord-check';
    } else {
      showAlert(alert, 'error', '❌ ' + (data.error || 'Ошибка сервера'));
    }
  } catch (err) {
